  ...
  return (
    <div className='flex shadow-lg justify-between p-2'>
        ...
        <div  className="flex w-full items-center">
            <input type="text" placeholder="Search" value={searchQuery} onChange={(e)=>{setSearchQuery(e.target.value)}} className='p-2 ml-4 w-3/4 border border-gray-400 rounded-l-full'></input>
        ...
        {(suggestions.length!=0)?(<div className='fixed bg-white shadow-lg px-2 py-2 ml-5 mt-2  rounded-lg w-[37%]'>
            <ul>
                {suggestions.map((s)=>(<li key={s} className='py-1 hover:bg-gray-100 px-1 rounded-lg'>{s}</li>))}
